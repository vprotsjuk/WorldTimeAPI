...
public class TimezoneAreaTest {
    ...
    public String getRandomParamsInWrongOrder() throws IOException {
        String endPoint = "/api/timezone";
        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                .url(baseURL + endPoint)
        ...
        if(index2 != -1) {
            param3 = param2.substring(index2 + 1);
            param2 = param2.substring(0, index+1);
        }
        List<String> intRand = new ArrayList<>();
        intRand.add(param);
        if (!param2.equals("")) {
            intRand.add(param2);
        }
        ...
        if (intRand.size() > 1) {
            while (intRand.get(0).equals(param)) {
        ...
        else if (intRand.size() == 3) {
            intRand.add(1, "/");
            intRand.add(3, "/");
        }
        String result = "";
        for(String s : intRand){
            result += s;
        }
    ...
    @Test
    @Description("Verify that response Area/Location/Region correspond with same request")
    public void requestCorrespondResponseVerifying() throws IOException {
        String endPoint = "/api/timezone/";
...
//        Random random = new Random();
        ...
        System.out.println(timezoneAreaDTO);
        int statusCode = response.code();


        sa.assertEquals(statusCode, 200);
        sa.assertEquals(timezoneAreaDTO.timezone, param);
        sa.assertTrue(timezoneAreaDTO.unixtime != 0L);
        sa.assertAll();
    }
    @Test
    @Description("Verify that wrong order of Area/Location/region is not accessible")
    public void wrongRequestOrderNotAccessible() throws IOException {
        String endPoint = "/api/timezone/";
        SoftAssert sa = new SoftAssert();
        Gson gson = new Gson();
        Random random = new Random();
        String param = getRandomParamsInWrongOrder();
        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                .url(baseURL + endPoint + param)
                .build();
...
//        System.out.println(timezoneAreaDTO);
        ...
        sa.assertEquals(statusCode, 404);
        sa.assertTrue(timezoneAreaDTO.timezone == null);
//        sa.assertTrue(timezoneAreaDTO.unixtime != 0L);
        sa.assertAll();
    }
